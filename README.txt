...
8) Add a subcategory: Send a post with endpoint #3 with json that includes {"name": "Example subcategory name"}. Name is required. 
9) Add a business: Send a post with endpoint #5 with json that includes {"name": "Example business name", "phoneNumber": "555-555-5555"} etc for name, phoneNumber, streetAddress, city, state, zipCode, hours, webpage. Names is required by database, all other fields are required by API so auto set them to be an empty string if the user doesn't input anything. 
...
17) Update/edit a business: Send a post to endpoint # 5. The json must include the ID, ex. {"id": 2} and at least one other field, as many other fields as they want to change. ex. {"id":2, "name": "Changing the name", "streetAddress": "Changing the street address"}
18) Add a subcategory to a category: Send a put to endpoint #7 using the IDs of the category and subcategory. 
19) Add a business to a subcategory: Send a put to endpoint #8 using the IDs of the subcategory and business. 
